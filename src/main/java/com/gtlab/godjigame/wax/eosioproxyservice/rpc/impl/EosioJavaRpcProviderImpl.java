package com.gtlab.godjigame.wax.eosioproxyservice.rpc.impl;

import com.google.gson.Gson;

import com.gtlab.godjigame.wax.eosioproxyservice.rpc.IEosioJavaRpcProviderApi;
import com.gtlab.godjigame.wax.eosioproxyservice.rpc.errors.EosioJavaRpcErrorConstants;
...
public class EosioJavaRpcProviderImpl implements IRPCProvider {
    ...
    @NotNull
    private final IEosioJavaRpcProviderApi rpcProviderApi;
    ...
    @Override
    public @NotNull GetBlockInfoResponse getBlockInfo(GetBlockInfoRequest getBlockInfoRequest) throws GetBlockInfoRpcError {
        var getBlockRequest = new GetBlockRequest(getBlockInfoRequest.getBlockNum().toString());
        try {
            var getBlockResponse = getBlock(getBlockRequest);

            final var getBlockInfoResponse = new GetBlockInfoResponse();
            var fields = getBlockInfoResponse.getClass().getDeclaredFields();

            for (var field : fields) {
                var sourceField = getBlockResponse.getClass().getDeclaredField(field.getName());
                sourceField.setAccessible(true);
                field.setAccessible(true);
                field.set(getBlockInfoResponse, sourceField.get(getBlockResponse));
            }

            return getBlockInfoResponse;
        } catch (GetBlockRpcError | NoSuchFieldException | IllegalAccessException e) {
    ...
    @Override
    public @NotNull SendTransactionResponse sendTransaction(SendTransactionRequest sendTransactionRequest) throws SendTransactionRpcError {
        var pushTransactionRequest = new PushTransactionRequest(sendTransactionRequest.getSignatures(),
            sendTransactionRequest.getCompression(), sendTransactionRequest.getPackagedContextFreeData(),
            sendTransactionRequest.getPackTrx());

        try {
            return pushTransaction(pushTransactionRequest);
        } catch (PushTransactionRpcError e) {
            throw new SendTransactionRpcError(e);
        }
    }
